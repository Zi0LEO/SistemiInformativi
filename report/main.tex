...
\begin{enumerate}
	\item Il valore dell'attributo \textit{data} all'interno dell'entità \textit{reso} deve essere posteriore al valore \textit{data di consegna} all'interno dell'entità \textit{Effettuata};
...
\begin{itemize}
  ...
  \item \textbf{Reso}(\underline{EmailCliente}, Data, Stato,CodiceSpedizione)\\ \textbf{Reso}(EmailCliente) $\subseteq _{FK}$ \textbf{Cliente}(Email); \\ \textbf{Reso}(CodiceSpedizione) $\subseteq_{FK}$ \textbf{Effettuata}(Codice)
\end{itemize}
\subsubsection{Triggers}
In merito al \textit{vincolo di integrità} 1 che tratta lo schema di relazione \textbf{Reso}(\underline{EmailCliente}, Data, Stato) ed \textbf{Effettuata}(Codice, EmailCliente), nel momento in cui all'interno dell'attributo data dell'entità \textbf{reso} si vuole inserire una nuova tupla bisogna controllare che la condizione per cui la \textit{data} che si sta cercando di inserire nel \textbf{reso} sia maggiore (posteriore) a quella che è attualmente all'interno di \textit{data consegna }di \textbf{effettuata}.
...
DELIMITER;
\end{minted}
 Un altro vincolo d'integrità è quello per valore dell'attributo \textit{stato} all'interno di \textbf{fattura}, nel caso in cui si vuole inserire un bisogna controllare che sia all'interno dell'insieme degli stati ammissibili.
 \begin{minted}[linenos]{sql}
 DELIMITER //
