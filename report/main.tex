...
\begin{enumerate}
  ...
  \item{(MUST)} Aggiornare lo stato di una spedizione:
    ...
    \end{enumerate}
  \item{(MUST)} Filtrare la lista degli indirizzi, la quale di default mostra tutti gli indirizzi, secondo i seguenti criteri[portalettere]:
    \begin{enumerate}
      \item{Comune di appartenenza};
      \item{Via di residenza}
      \item{Orario preferito per la consegna};
    \end{enumerate}
  \item{(MUST)} Ordinare la lista degli indirizzi, di default in ordine alfabetico, secondo i seguenti criteri[portalettere]:
    \begin{enumerate}
      \item{Nome del cliente};
    \end{enumerate}
  \item{(MUST)} Filtrare la lista delle spedizioni, la quale di default mostra tutte le spedizioni, secondo i seguenti criteri:
    \begin{enumerate}
      \item{Sopra o sotto una soglia di prezzo};
      \item{Per indirizzo di consegna};
      \item{Per indirizzo di partenza};
      \item{Per stato della spedizione};
    \end{enumerate}
  \item{(MUST)} Ordinare la lista delle spedizioni, di default ordinata in base al codice, secondo i seguenti criteri[portalettere]:
    \begin{enumerate}
...
\begin{itemize}
  \item L'attributo prezzo dell'entità Spedizione è calcolato tramite una tabella di prezzi basata ai pesi.
...
\begin{table}[H]
    ...
    \hline
     Amministratore & Rappresenta gli utenti con ogni permesso & Quelli dell'entità madre & E-mail entità madre \\
    \hline
     Dipendente & Rappresenta ogni dipendente con accesso al sistema & Quelli dell'entità madre, permessi & E-mail entità madre\\
    \hline
		 Cliente & Rappresenta i clienti dell'organizzazione & Quelli dell'entità madre & E-mail \\
		\hline 
...
\begin{itemize}
  \item \textbf{Cliente}(\underline{E-mail}, Nome, Cognome);
  \item \textbf{Indirizzo}(\underline{Comune, Via, Civico}, Orario, EmailCliente) \\ \textbf{Indirizzo}(EmailCliente) $\subseteq _{FK}$ \textbf{Cliente}(E-mail); 
  \item \textbf{Ricevuta}(\underline{Codice}, Stato, Data)\\ \textbf{Ricevuta}(Codice) $\subseteq _{FK}$ \textbf{Spedizione}(Codice);
  \item \textbf{Spedizione}(\underline{Codice}, Peso, Prezzo, EmailMittente, EmailDestinatario) \\ \textbf{Spedizione}(EmailMittente) $\subseteq _{FK}$ \textbf{Cliente}(E-mail) \\ \textbf{Spedizione}(EmailDestinatario) $\subseteq _{FK}$ \textbf{Cliente}(E-mail);
...
\end{minted}
I metodi di tipo creazione devono anche essere transition safe tramimte l'oggetto \textit{Transition} di Torque e la possibilità di fare una \textit{rollback} nel caso in cui all'intero del try and catch c'è un'errore, oltretutto si deve fare una check del caso in cui il cliente che si sta cercando di inserire è già esistente, visto che la funzione \textit{save()} di torque non fa questa cosa.
...
\end{figure}
Qui ci sono diverse opzioni, partendo dal basso si possono vedere i propri dati in merito alle spedizioni di vario tipo premendo il bottone corrispondente, si può ricevere la lista delle ricevute premento l'apposito bottone \textit{Ricevute} che modificherà la list presente sotto. Il bottone \textit{Scarica documenti per reso} automaticamente genererà un file pdf che conterrà un codice a barre che dovrà essere compilato per far partire la procedura. 
I più in alto da sinistra c'è il bottone che permette di tornare indietro alla schermata precedente \textit{Logout}, quello che permette di eliminare il proprio account \textit{Elimina Account} e quello che permette di modificare i propri dati, \textit{Modifica dati}, quest'utlimo aprirà una scheda aggiuntiva:
\begin{figure}[H]
	\includegraphics[width=\textwidth]{assets/modification_page.png}
	\caption{Schermata principale con modifica}
\end{figure}
Qui compilando i vari box di testo sarà possibile modificare i propri dati all'interno della base di dati.
\begin{figure}[H]
...
La base di dati è appoggiata su MySQL, ma in Java è implementata tramite \textit{Torque}, quindi tramite un'approccio di tipo ORM, le classi da lui generate sono quindi degli accessi diretti alle entità.
\subsection{UI}
